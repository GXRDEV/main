...
function seriesHandler(transaction, results) {
    ...
    for(var i=0; i<results.rows.length; i++) {
        ...
        if(isCompatible()) {
            ...
            var refImgSrc = location.origin + '/dcmimage?serverURL=';
...
function imageHandler() {
	...
	if(instanceNo != 'null' && instanceNo != null && instanceNo.length>0) {
		imgInc = parseInt(instanceNo);
	}
...
function getActiveFrame() {
	//liufz
	var _frame = jQuery(document).find('iframe');
    var frames = _frame.size() ? _frame : jQuery(parent.document).find('iframe');
...
function windowLevelHandler(data, totalInstances) {
	...
	if(!!data['imageOrientation'] && data['imageOrientation'] != 'undefined') {
    ...
    if(!!data['numberOfFrames'] && data['numberOfFrames'] != 'undefined') {
        if(parent.scrollImages || jQuery('#loopChkBox').is(':checked')) {
    ...
    if(!!data['sliceThickness'] && data['sliceThickness'] != 'undefined') {
        sliceInfo = 'Thick: ' + parseFloat(data['sliceThickness']).toFixed(2) + ' mm ';
    }

    if(!!data['sliceLocation'] && data['sliceLocation'] != 'undefined') {
    ...
    if(!!data['frameOfReferenceUID'] && data['frameOfReferenceUID'] != 'undefined') {
        if(parent.scrollImages || jQuery('#loopChkBox').is(':checked')) {
    ...
    if(!!data['refSOPInsUID'] && data['refSOPInsUID'] != 'undefined') {
        if(parent.scrollImages || jQuery('#loopChkBox').is(':checked')) {
    ...
    if(parent.scrollImages || jQuery('#loopChkBox').is(':checked')) {
        ...
        imgSrc = location.origin + '/dcmimage?serverURL=' + serverURL + '&study=' + studyUID + '&series=' + seriesUID + '&object=' + data['sopUID'];
...
function setSeriesIdentification() {
	...
	for(var x=0;x<framesCnt.size();x++) {
		if(!!jQuery(framesCnt[x]).contents().find('#frameSrc').html()){
			var fsrc = jQuery(framesCnt[x]).contents().find('#frameSrc').html();
			var serUidTmp = getParameter(fsrc,'seriesUID');
