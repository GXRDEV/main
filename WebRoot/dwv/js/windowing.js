...
function parseAndLoadDicom()
{
    ...
    if( !(!(wadoURL.indexOf('C-GET') >= 0) && !(wadoURL.indexOf('C-MOVE') >= 0))) {
        ...
        var urlTmp = location.origin + "/Wado.do?study=" + getParameter(wadoURL, "studyUID") + "&object=" + getParameter(wadoURL, "objectUID") + "&contentType=application/dicom";
    	reader.readDicom(urlTmp);
    } else {
    	 reader.readDicom(location.origin + "/DcmStream?wadourl="+wadoURL.replaceAll("&","_"));
    }
...
function retrieveImage1(studyUID, seriesUID, instanceUID) {
    ...
    //var url = location.origin + '/dcmimage?serverURL=http://' + host + ':' + wadoPort + '&study=' + studyUID + '&series=' + seriesUID + '&object=' + instanceUID;
    var url = location.origin + '/dcmimage?serverURL=' + parent.wadoURL.substring(0, parent.wadoURL.indexOf('wado')-1) + '&study=' + studyUID + '&series=' + seriesUID + '&object=' + instanceUID;
