...
(function ($) {
        ...
        */
        diyUpload: function (opt, serverCallBack) {
            ...
            webUploader.on('uploadSuccess', function (file, response) {
                ...
                $fileBox.find('.viewThumb img').attr('data-id',response);
                $diyBar.hide();                
                procs && $fileBox.fadeOut();
                $fileBox.nextAll().filter(function(){
                	return $(this).hasClass('diyUploadHover') || $(this).find('.webuploader-pick').size();
                }).show();
                if (successCallBack) {
    ...
    function createBox($fileInput, file, webUploader) {
        ...
        var $diyCancel = $fileBox.children('.diyCancel').one('click', function () {
        	...
        	hidden.val(hs.join(','));
        	SetMoveState1 && SetMoveState1(ul,true);
        });
