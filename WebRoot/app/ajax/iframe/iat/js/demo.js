...
var iflytek = (function(document){
    ...
    var tiptxt = document.getElementById('b');
        ...
        "callback":{
            "onResult": function (err, result) {
                ...
                if (err == null || err == undefined || err == 0) {
                    ...
                    window.parent.Page && window.parent.Page.dialog && window.parent.Page.dialog.luyin ?
                    	window.parent.Page.dialog.luyin(result) :
                    	(iat_result.style.display = 'block');
                } else {
                    iat_result.innerHTML = 'error code : ' + err + ", error description : " + result;
                    console.log(iat_result.innerHTML);
                }
                mic_pressed = false;
                volumeEvent.stop();
            },
            ...
            "onProcess":function(status){
                switch (status){
                    case 'onStart':
                    	tiptxt.innerHTML = "服务初始化...";
                        break;
                    case 'normalVolume':
                    case 'started':
                    	tiptxt.innerHTML = "倾听中...";
                        break;
                    case 'onStop':
                    	tiptxt.innerHTML = "等待结果...";
                        break;
                    case 'onEnd':
                    	tiptxt.innerHTML = oldText;
                        break;
                    case 'lowVolume':
                    	tiptxt.innerHTML = "倾听中...(声音过小)";
                        break;
                    default:
                    	tiptxt.innerHTML = status;
                }
            }
        }
    });
    ...
    var play = function() {
        if (!mic_pressed) {
            var ssb_param = {
                "grammar_list": null,
                "params": "appid=583f98ab,appidkey=81b9fc3326be2ff4, lang = sms, acous = anhui, aue=speex-wb;-1, usr = mkchen, ssm = 1, sub = iat, net_type = wifi, rse = utf8, ent =sms16k, rst = plain, auf  = audio/L16;rate=16000, vad_enable = 1, vad_timeout = 5000, vad_speech_tail = 500, compress = igzip"
            };
