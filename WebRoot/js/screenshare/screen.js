...
(function() {
    ...
    window.Screen = function(channel) {
        ...
        this.onscreen = function(screen) {
        	 ...
        	 console.log('detectedRoom:' + self.detectedRoom);
    ...
    function Signaler(root, roomid) {
        ...
        this.broadcast = function(_config) {
            ...
            (function transmit() {
                ...
                if (!signaler.stopBroadcasting && !root.transmitOnce)
                    setTimeout(transmit, 3000);//毫秒数  没3秒发一次
            })();
        ...
        function leaveRoom() {
            ...
            if (root.stream){
            	if(root.stream.stop) {
            		root.stream.stop(); // FF and other not yet updated browsers
                } else {
                    // updated browsers
                    var tracks = root.stream.getTracks();
  ...
  /*  var iceFrame, loadedIceFrame;
    ...
    function onOfferError(e) {
        console.error('offer error:', e);
    }
    
    function onAnswerError(e) {
    ...
    var Offer = {
        createOffer: function(config) {
            var peer = new RTCPeerConnection(window.IceServers, optionalArgument);
    ...
    var Answer = {
        createAnswer: function(config) {
            ...
            peer.createAnswer(function(sdp) {
                ...
                peer.setLocalDescription(sdp);
                config.onsdp(sdp, config.to);
            }, onAnswerError, answerConstraints);
