...
define('Tables',{
	...
	"day":[{key:"1",value:"上午"},{key:"2",value:"下午"},{key:"3",value:"晚上"}],
	"state":[{key:"1",value:"开启"},{key:"0",value:"关闭"}],
	"outtype":[{key:"1",value:"专家号"},{key:"0",value:"普通号"}],
	"RemoteTime":[{type:"select",name:"week",option:"week"},{type:"select",name:"outTime",option:"day"},{type:"select",name:"outType",option:"outtype"},{type:"text",name:"cost"},{type:"checkbox",name:"status",option:"state"},{type:"text",name:"address"}],
...
define('Controller',['view/base','view/date'],{
	...
	saveForm:function(tr,callback){
		...
		action && base.showTipIngA('操作执行中……').ajax(action,inputs,function(data){
			if(data.status=='success'){
				...
				base.showTipE('有相同日期相同时段的启用出诊时间');
			}
		},function(){
			base.showTipE('操作失败');
			callback(id);
	...
	delTR:function(tr,callback){
		...
		delurl="doctor/deldocscheduleshow";
