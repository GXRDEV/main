    ...
    <div id="index">
    	<header>
    		<p class="tabs">
				...
				<a href="/wzjh/mycancelorders?openid=${openid}"   class="current">已取消</a>
    		</p>
    	</header>
    		...
    		<c:forEach items="${orders}" var="order">
    			<li class="orderList">
	    			<div class="headTop box">
	    				<label>订单ID&ensp;${order.id}</label>
	    				<div class="cols1">
	    					<c:choose>
   								<c:when test="${order.status==20}">用户取消</c:when>
   								<c:when test="${order.status==21}">专家取消</c:when>
	    			...
	    			<div class="bodyDiv">
	    				<dl>
	    					...
	    					<dd><label>取消时间</label><span><fmt:formatDate value="${order.refreshTime}" pattern="yyyy-MM-dd HH:mm:ss" /></span></dd>
		...
		function moreInfo(id,more,p){
			...
			_b._post('wzjh/moreorders',{openid:_b.oid,flag:_b.flg,pageNo: nextP},function(d){
				...
				d.orders.length ? ($.each(d.orders,function(i,o){
					var fail = o.status==20?'用户取消':'专家取消';
					var _date=datestring(o.refreshTime);
					
					ops += '<li class="orderList">\
				    			<div class="headTop box">\
				    				<label>订单ID&ensp;'+ o.id+'</label>\
				    				<div class="cols1">'+ fail +'</div>\
				    			</div>\
				    			<div class="bodyDiv">\
				    				<dl>\
				    					<dd><label>患者姓名：</label><span>' + o.patientName  + '</span></dd>\
	    								<dd><label>取消时间</label><span>'+ _date +'</span></dd>\
