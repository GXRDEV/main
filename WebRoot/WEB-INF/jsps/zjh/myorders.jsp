...
<html>
  ...
  <body>
    <div class="index">
    	<header>
    		<p class="tabs">
    			<a href="/wzjh/myorders?openid=${openid}" class="current">进行中</a>
				<a href="/wzjh/mycompletedorders?openid=${openid}">已完成</a>
	    				...
	    				<div class="cols1">
	    					<c:choose>				
	    						<c:when test="${order.status==2}">
	    							<a class="btnTop" href="/wzjh/specialdetail?openid=${openid}&sid=${order.expertId}&cooHosId=${order.localHospitalId}&orderId=${order.id}" >二次下单</a>
	    						</c:when>
	    						<c:when test="${order.status==3}">
	    							<a class="btnTop" href="/wzjh/specialdetail?openid=${openid}&sid=${order.expertId}&cooHosId=${order.localHospitalId}&orderId=${order.id}" >三次下单</a>
	    						</c:when>
	...
	<script>
    	var _b = {
    		...
    		cancel:'<p><b>是否确定要取消本次会诊服务？</b><br/>取消后，所有款项自动退回到您的付款帐户内。</p><div><a href="javascript:void(0);" style="color:#353535;" onclick="hidePop()"> 取 消 </a><a href="javascript:void(0);" style="color:#10B5F0;" id="cancelDeal"> 确 定 </a></div>',
    	};	
    	$(function(){
    		$('body').delegate('#cancelDeal','click',function(){
    			var oid = $(this).attr('data-action');
    			oid &&( $(this).html(_b.limg),_b._post(oid,{},function(d){
					_gr(_b.href + 'wzjh/myorders?openid=${openid}');
				}));
    		}).delegate('#cancelDeal2','click',function(){
    			var oid = $(this).attr('data-action');
    			oid && _gr(_b.href + oid);
    		}).delegate('[data-address]','click',function(){
		...
		function showPop(oid,t,orderid){
			$("#seloid").val(orderid);
			_b.dia.alert.show({text:t ? (oid != 0 ? _b.ok : _b.deal) : _b.cancel});
			$('#cancelDeal' + (t ? (oid != 0 ? '2' : '3') : '')).attr('data-action',oid);
		}			
		function hidePop(){
			_b.dia.alert.hide();
		}
		function moreInfo(id,more,p){
			...
			_b._post('wzjh/moreorders',{openid:_b.oid,flag:_b.flg,pageNo: nextP},function(d){
				...
				d.orders.length ? ($.each(d.orders,function(i,o){
					...
					var aa = '<a class="btnTop" href="'+ _url +'wzjh/specialdetail?openid=${openid}&sid='+o.expertId+'&cooHosId='+o.localHospitalId+'&orderId='+o.id+'" >{0}</a>';
					switch(sign){
						case 2:	condate=o.consultationDate;btn = aa.replace('{0}','二次下单');break;
						case 3:	condate=o.secondConsultationDate;btn= aa.replace('{0}','三次下单');break;
