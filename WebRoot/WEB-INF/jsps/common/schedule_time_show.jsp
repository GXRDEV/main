				...
				<div class="row-fluid">
					<div class="span12">						
						<div class="widget-box" id="models_show">
							...
							<div class="widget-content nopadding" data-tablespace="RemoteTime" data-action="/doctor/sudocscheduleshow">
								<table class="table1">
									<thead>
										<tr>
											<th><span class="td">时间（周）</span></th>
											<th><span class="td">时段</span></th>
											<th><span class="td">类别</span></th>
											<th><span class="td">费用</span></th>
											<th><span class="td">状态</span></th>
											<th><span class="td">出诊地点</span></th>
									...
									<tbody>
										<c:forEach items="${times}" var="time">
											<tr id="${time.id}">
												...
												<td><span class="td"><label class="text" data-val="${time.outTime}" data-option="day">${time.outTime}</label></span></td>
												<td><span class="td"><label class="text" data-val="${time.outType}" data-option="outtype">${time.outType}</label></span></td>
												<td><span class="td"><label class="text" data-val="${time.cost}">
													<c:if test="${empty time.cost }">0</c:if>
													<c:if test="${!empty time.cost }">${time.cost }</c:if>
													</label></span></td>
												<td><span class="td"><label class="text" data-val="${time.status}" data-option="state">${time.status}&nbsp;</label></span></td>
												<td><span class="td"><label class="text" data-val="${time.address}">${time.address}&nbsp;</label></span></td>
