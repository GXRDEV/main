	...
	<script>
            ...
            $(document).ready(function () {            	
            	oTable = $('.data-table').DataTable({
                    ...
                    "sAjaxSource": "/expert/gaintelorders",
                    "bFilter": true,"bLengthChange": true,"bProcessing": true,   
                    "columns":[
                          		null,null,null,null,null,null,null,null
                           ],
					"fnCreatedRow": function (nRow, aData, iDataIndex) {
						var path= '/expert/teldetail/' + aData[0],_val=aData[5];
						var ops='';
						if(_val.indexOf('待接诊') != -1){	
							ops+='<a href="javascript:void(0);" data-id="'+ aData[0] +'" class="linebtn telclose" close-data="20">接诊</a>';
							ops+='<a href="javascript:void(0);" data-id="'+ aData[0] +'" class="linebtn telclose" close-data="30">拒绝</a>';
						}
                    	$('td:eq(7)', nRow).html('<a href="'+path+'" data-id="'+ aData[0] +'" class="editbtn linebtn">查看详情</a>'+ops);
                    },
                    "oLanguage": {
            	...
            	$('body').delegate('#filteropt a','click',function(){
                	...
                	oTable.page('first').draw(false);
                	return false;
                }).delegate('.telclose','click',function(){
                	var oid=$(this).attr("data-id"),ctype=$(this).attr("close-data");
                	$.post('/doctor/closetel',{oid:oid,ctype:ctype},function(data){
                		oTable.page('first').draw(false);
                	});
