	...
	<script>
            ...
            $(document).ready(function () {   
            	...
            	oTable = $('.data-table').DataTable({
                    ...
                    "sAjaxSource": "/hospital/gaininvitjoindatas",
                    ...
                    "initComplete": function(settings, json) {
												var tabs;
												tabs='<a href="#" class="stalist js-addinfo selected" data-ostatus="1">我发起的邀请申请</a>';
												tabs+='<a href="#" class="stalist js-addinfo"  data-ostatus="2">我收到的邀请请求</a>';
												$('#filteropt').html(tabs)
                    }
            	});
							$('body').delegate('#filteropt a','click',function(){
									opt = $(this).attr('data-ostatus');
									opt == 1? $('.hosTitle').html('被邀请医院'): $('.hosTitle').html('上级医院')
									$(this).addClass('selected').siblings().removeClass('selected');
									oTable.page('first').draw(false);
									return false;
							}).delegate('.changestatus', 'click', function(){
									var aid=$(this).attr('aid'),sval=$(this).attr('sval');
									if(confirm("请确认是否更改医院的邀请加入医联体的状态？！")){	
                		$.post('/hospital/changeappstatus',{aid:aid,sval:sval,stype:2},function(data){
                    		oTable.page('first').draw(false);
                    	});
                	}
							})
