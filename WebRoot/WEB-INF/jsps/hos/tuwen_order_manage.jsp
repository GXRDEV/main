...
<body>
    <div class="mainlist">
        <div class="container-fluid" style="padding-top: 16px;">
            <div class="fliters">
                <a href="javascript:;;" class="selected" data-sign="1">发起的</a>
                <a href="javascript:;;" data-sign="2">接收的</a>
            </div>
            <div class="row-fluid">
                <div class="span12">
    ...
    <script>
        ...
        $(document).ready(function () {
            oTable = $('.data-table').DataTable({
                ...
                "sAjaxSource": "/hospital/gaintuwenorderdatas",
                "bFilter": true,
                "bLengthChange": true,
                "bProcessing": true,
                "fnCreatedRow": function (nRow, aData, iDataIndex) {
                    var opts = '';
                    opts += '<a href="/app/main.html#ajax/admin/detail.html?oid=' +
                        aData[0] + '&otype='+ otype +'" class="linebtn">查看详情</a>';
                    if ($('.fliters a.selected').attr('data-sign') == '1') {
                        if ($('#filteropt a.curr').attr('order-status') == undefined ||
                            $('#filteropt a.curr').attr('order-status') == '1') {
                            opts +=
                                '<a href="javascript:void(0);" onclick="showDialog(' +
                                aData[0] + ')" class="distDoc linebtn">分配医生</a>';
                            opts += '<a href="javascript:void(0);" id="' + aData[0] +
                                '" sval="50" class="changstat linebtn">取消订单</a>';
                        }
                        ...
                        if ($('#filteropt a.curr').attr('order-status') == undefined ||
                            ...
                            opts +=
                                ...
                                aData[0] + ')" class="distDoc linebtn">分配医生</a>';
                            opts += '<a href="javascript:void(0);" id="' + aData[0] +
                                '" sval="30" class="changstat linebtn">退诊</a>';
                        }
                        if ($('#filteropt a.curr').attr('order-status') == '2') {
                            opts += '<a href="javascript:void(0);" id="' + aData[0] +
                                '" sval="30" class="changstat linebtn">退诊</a>';
                ...
                "initComplete": function (settings, json) {
                    var tabs =
                        '<a href="#" class="stalist curr" order-status="1">待分配</a>';
            ...
            $('body').delegate('#filteropt a', 'click', function () {
                ...
                var tuwenId = $(this).attr('id');
                $.post('/hospital/delTuwen', {
                    tuwenId: tuwenId
                }, function (d) {
                    oTable.draw(false);
                });
                ...
                $('#myform tbody').html(function () {
                    ...
                    $.each(d.doctors || [], function (i, n) {
                        ...
                        if (n.openAsk == 1) {
                            ...
                            openser =
                                '<a style="color:red" href="/app/main.html#ajax/doc/server.html?docid=' +
                                n.specialId + '">未开通</a>';
                        }
                        label +=
                            '<tr><th><input type="radio" name="doclist" id="doc' +
                            n.specialId + '" value="' + n.specialId +
                            '"></th><td><label for="doc' + n.specialId +
                            '">' +
                            n.specialName +
                            '</label></td><td><label for="doc' + n.specialId +
                            '">' + n.depName +
                            '</label></td><td><label for="doc' + n.specialId +
                            '">' + openser + '</label></td></tr>';
                    });
        ...
        function retrieveData(sSource, aoData, fnCallback) {
            ...
            aoData.push({
                "name": "dtype",
                "value": $('.fliters a.selected').attr('data-sign') || ''
            });
            aoData.push({
                "name": "ostatus",
                "value": $('#filteropt a.curr').attr('order-status') || '1'
            });
            $.ajax({
                "type": "post",
