	...
	<script>
            ...
            $(document).ready(function () {            	
            	oTable = $('.data-table').DataTable({
                    ...
                    "sAjaxSource": "/system/d2preferdatas",
                    ...
                    "columnDefs": [
                    	{
                   		...
                   		{
                   			"targets": [2, 3, 4, 6, 7,8,10,11,12], "visible": false
                   		},
                   		{
                   			"targets": 5,
                   			"render": function ( data, type, row, meta ) {	                  
                   		...
                   		},
                   		{
                   			"targets": 9,
                   			"render": function ( data, type, row, meta ) {	                  
						    	return row[9] + '<i>/</i>' + row[10] + '<i>/</i>' + row[11]+ '<i>/</i>'+row[12];
						    }
                   		}
					],
					"fnCreatedRow": function (nRow, aData, iDataIndex) {
						
						var opts='<a href="/app/main.html#ajax/admin/admin_detail.html?orderId='+aData[0]+'&&orderType=10" class="editbtn linebtn">查看详情</a>';
						if(aData[17]=='已退诊'||aData[17]=='已取消'){
							opts+='<a href="javascript:void(0);" class="statusup linebtn" val="delete" oid='+aData[0]+'>订单删除</a>';
						}
						if(aData[17]=='待接诊'){
							opts+='<a href="javascript:void(0);" class="statusup linebtn" val="out" oid='+aData[0]+'>订单退诊</a>';
						}
						if(aData[17]=='已接诊'){
            	...
            	$('body').delegate('#filteropt a','click',function(){
                	...
                	if(confirm("您正在变更订单状态，请确认是否继续操作！")){
                		...
                		$.post('/system/changeOrderStatus',{oid:oid,sval:sval,otype:"10",refuls:refuls},function(data){
                    		oTable.draw(false);
                    	});
                	}
